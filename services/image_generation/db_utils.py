...
async def get_detailed_matches(tag):
    async with SessionLocal() as session:
        repo = StatsRepository(session)
        return await repo.get_detailed_matches(tag, limit=10000000) # arbitrary large limit to fetch all
...
async def fetch_detailed_data_for_matches(tag):
    overall_stats, matches = await asyncio.gather(
        get_overall_stats(tag),
        get_detailed_matches(tag), # arbitrary lim grater than the amount of brawlers
    )
